...
.. decorator:: cache(timeout)
    ...
    Same as :func:`memoize`, but doesn't use cached results older than ``timeout``. It can be either number of seconds or :class:`datetime.timedelta`. Also, doesn't support skipping.
